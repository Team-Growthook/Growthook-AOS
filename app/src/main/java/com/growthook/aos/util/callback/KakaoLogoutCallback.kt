...
class KakaoLogoutCallback(private val onSuccess: (isSuccess: Boolean) -> Unit) {
    fun handleResult(error: Throwable?) {
        if (error != null) {
            when {
                ...
                error.toString() == AuthErrorCause.Unauthorized.toString() -> {
                    ...
                    Timber.e(error, "$error 기타 에러")
                }
            }
        } else {
            Timber.d("로그아웃 or 탈퇴 성공")
            onSuccess(true)
        }
    }
}
